...
function coordinatesFromContext(base: RepoCoordinates, context: unknown): RepoCoordinates {
  ...
  return {
    ...base,
    ref: typeof value.ref === 'string' ? value.ref : base.ref,
    target,
    accessMode: access === 'read-only' || access === 'write-capable' ? access : base.accessMode,
  };
}
...
function withoutEmbeddedContext(result: unknown) {
  if (typeof result !== 'object' || result === null || Array.isArray(result) || !('context' in result)) return result;
  const { context: _context, ...rest } = result as Record<string, unknown>;
  return rest;
}
...
export const forgeRegistry = {
  ...
  describe(name: string) {
    return capabilityDescription(name);
  },
  ...
  async executeForAgent(
    ...
    // context.snapshot already is the requested context. Never duplicate it in
    // an outer context field.
  ...
  async executeBatchForAgent(
    ctx: ForgeCapabilityContext,
    items: ForgeBatchItem[],
    contextRequest: AgentContextRequest = { mode: 'none' },
  ) {
    ...
    while (index < items.length) {
      ...
      if (mutates) {
        results[index] = await this.executeForAgent(ctx, item.name, item.input ?? {}, contextRequest);
        index += 1;
        continue;
      }
      ...
      const resolved = await Promise.all(group.map((entry) => this.executeForAgent(
        ctx,
        entry.name,
        entry.input ?? {},
        contextRequest,
